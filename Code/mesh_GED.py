...
def compute_graph_edit_distance(DRG1, DRG2):
    """
    Compute Graph Edit Distance (GED) between two Decorated Reeb Graphs (DRGs).
    """
    G1 = DRG1.ReebGraph  # Extract the NetworkX graph
    G2 = DRG2.ReebGraph  # Extract the NetworkX graph

    def node_match(node1, node2):
        # Match nodes based on position ('pos') or persistence diagram ('diagram')
        pos_match = np.allclose(node1.get('pos', [0, 0, 0]), node2.get('pos', [0, 0, 0]), atol=1e-2)
        return pos_match

    def edge_match(edge1, edge2):
        # Match edges based on weight
        weight1 = edge1.get('weight', 0)
        weight2 = edge2.get('weight', 0)
        return np.isclose(weight1, weight2, atol=1e-2)

    # Optimize Graph Edit Distance
    ged = nx.optimize_graph_edit_distance(G1, G2, node_match=node_match, edge_match=edge_match)

    # Return the first (or best) distance found
    return next(ged)
...
def evaluate_knn(test_DRGs, train_DRGs, train_labels, test_labels, k_values):
    ...
    for test_DRG in tqdm(test_DRGs, desc="Calculating distances for test DRGs"):
        distances = [
            compute_graph_edit_distance(test_DRG, train_DRG)  # Use the updated function
...
if __name__ == "__main__":
    ...
    accuracies, eval_time = evaluate_knn(all_DRGs[:10], all_DRGs, encoded_labels_extended, encoded_labels_extended[:10],
                                         k_values)
